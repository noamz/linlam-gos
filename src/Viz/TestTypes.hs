...
main = do
  ...
  let mkDiagram t =
        let tp = Lam.synthClosedNormal t in
        hsep 1 [vsep 1 [diagULT b t # centerX, text (Lam.prettyULT t) # centerX ],
                text ":",
                vsep 1 [diagType b tp # centerX, text (Lam.prettyType tp) # centerX]]
