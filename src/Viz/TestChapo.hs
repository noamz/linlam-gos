...
main = do
  ...
  let tminterval t = (Ch.lams2dowLR t,Ch.apps2tree t)
  let d = vsep 1 $ labelledVList [(diagTree c # scale 2,
                                   hsep 3 $ labelledHList $ zip (map (\t -> vsep 1 [diagArcs (Ch.lams2dowLR t) # centerX, text (Lam.prettyULT t) # centerX ]) ts) (repeat mempty)) | (c,ts) <- byApps'']
  mainWith (d # frame 1)
